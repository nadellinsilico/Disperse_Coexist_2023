...
hfig = figure;
v = VideoWriter('spatial_biased_global_dispersal_long_fig2.mp4', 'MPEG-4');
v.FrameRate = 5;
open(v);
Nt = int16(Tend / dt);
...
% dispersal of each species, edit dispersal bias here.
spa = 0.2;
sef = 0;
sec = 0;
...
if sum(pa) >= pa_thresh_up*51*51
    ss(:) = 1;
elseif sum(pa) <= pa_thresh_down*51*51 
    ss(:) = 0;
else
    ss(:) = s(:);
end
